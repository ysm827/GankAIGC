...
- If a task is already on the strongest strategy, has repeated stagnation, and a strict rollback still leaves the risk above threshold, stop the loop with a compact `type="plateau_exit"` trace event instead of burning more Zhuque/beer quota. The task remains `failed`, the best saved reduced text is preserved, and the diagnosis must tell the user to manually adjust stubborn paragraphs or relax the threshold near the detector floor.
...
- Good: A session stuck near 25% after repeated strongest-strategy rollbacks records `type="plateau_exit"`, preserves the best text, and fails with an actionable manual-review diagnosis instead of continuing redundant rounds.
